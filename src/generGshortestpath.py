...
def GenerWeightedG(data3):
    ...
    return Gcity
def GenerCountG(data3):
    Gcity=nx.Graph()
    for i in data3.index:
        Gcity.add_edge( data3['departure'][i],data3['destination'][i],weight=data3[1][i])
    return Gcity

def GenerLogG(data3):
    Gcity=nx.Graph()
    for i in data3.index:
        Gcity.add_edge( data3['departure'][i],data3['destination'][i],weight=data3['log'][i])
    return Gcity
...
def drawGraph(true,sp,wsp,cp,lp):
    import gmplot
    import numpy as np
    truth_path = np.array(zipcodeToLL(true))
    sp_path = np.array(zipcodeToLL(sp))
    cp_path = np.array(zipcodeToLL(cp))
    lp_path = np.array(zipcodeToLL(lp))
    ...
    gmap.plot(wsp_path[:, 0], cp_path[:, 1], 'black', edge_width=1)
    gmap.plot(wsp_path[:, 0], lp_path[:, 1], 'grey', edge_width=1)
    gmap.draw("../out/compare.html")

# data3=pd.read_csv("../out/graph.csv" )
G=GenerG(data3)
WG = GenerWeightedG(data3)
CG = GenerCountG(data3)
LG = GenerLogG(data3)
result = []
weighted_result = []
cg_result = []
...
drawGraph(valid_trips[200],sp[200],wsp[200],cp[200],lp[200])
# result = np.array(result)
# weighted_result = np.array(weighted_result)
# print(result.mean())
# print(weighted_result.mean())
cg_result = np.array(cg_result)
print(cg_result.mean())